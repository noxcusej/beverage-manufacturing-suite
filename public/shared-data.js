...
const CoManufacturing = {
    ...
    getFormulas() {
        ...
        let changed = false;
        formulas.forEach(f => {
            if (!f.id) { f.id = this.generateFormulaId(); changed = true; }
        });
        if (changed) {
    ...
    saveFormula(formula) {
        ...
        if (existingIdx !== -1) {
            ...
            savedFormula = {
                ...formula,
                id: formula.id || this.generateFormulaId(),
                versions: [],
                createdAt: now,
                updatedAt: now,
            };
        ...
        this._syncFormulaToSupabase(savedFormula);
    },
    ...
    async _syncFormulaToSupabase(formula) {
        if (!this._supabaseReady || !this._supabase) return;
        try {
            ...
            const { error } = await this._supabase.from('formulas').upsert(row, { onConflict: 'id' });
            if (error) console.error('[Supabase] saveFormula error:', error.message);
        } catch (err) {
            console.error('[Supabase] saveFormula exception:', err);
        }
    },

    async hydrateFormulasFromSupabase() {
        if (!this._supabaseReady || !this._supabase) return;
        try {
            ...
            if (!data || data.length === 0) {
                // Push local formulas to Supabase
                const local = this.getFormulas();
                if (local.length > 0) this._syncAllFormulasToSupabase(local);
                return;
            }
            ...
            localStorage.setItem(this.STORAGE_KEYS.formulas, JSON.stringify(merged));
            this.notifyChange('formulas');

            if (localOnly.length > 0) {
                this._syncAllFormulasToSupabase(localOnly);
            }
        } catch (err) {
            console.error('[Supabase] hydrateFormulas exception:', err);
        }
    },

    async _syncAllFormulasToSupabase(formulas) {
        if (!this._supabaseReady || !this._supabase) return;
        try {
            const rows = formulas.map(f => {
                const { versions, id, name, client, createdAt, updatedAt, ...rest } = f;
                return { id, name, client: client || 'Uncategorized', data: rest, versions: versions || [] };
            });
            const { error } = await this._supabase.from('formulas').upsert(rows, { onConflict: 'id' });
            if (error) console.error('[Supabase] syncAll error:', error.message);
        } catch (err) {
